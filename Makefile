# This Makefile is for the File::Slurp::Affix extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/File/Slurp/Affix.pod]
#     AUTHOR => [q[Lady Aleena <aleena@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0] }
#     LICENSE => q[artistic_2]
#     MIN_PERL_VERSION => q[5.006]
#     NAME => q[File::Slurp::Affix]
#     PREREQ_PM => { Encode=>q[0], Exporter=>q[0], File::Temp=>q[0], Test::More=>q[0] }
#     TEST_REQUIRES => { Encode=>q[0], File::Temp=>q[0], Test::More=>q[0] }
#     VERSION_FROM => q[lib/File/Slurp/Affix.pm]
#     clean => { FILES=>q[File-Slurp-Affix-*] }
...
MAKEMAKER   = /home/me/perl5/lib/perl5/ExtUtils/MakeMaker.pm
MM_VERSION  = 7.44
MM_REVISION = 74400
...
MAKE = make
FULLEXT = File/Slurp/Affix
BASEEXT = Affix
PARENT_NAME = File::Slurp
DLBASE = $(BASEEXT)
VERSION_FROM = lib/File/Slurp/Affix.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/File/Slurp/Affix.pod

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/File/Slurp
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/File/Slurp
...
TO_INST_PM = lib/File/Slurp/Affix.pm \
	lib/File/Slurp/Affix.pod
...
DISTNAME = File-Slurp-Affix
DISTVNAME = File-Slurp-Affix-0.01_01
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/File/Slurp/Affix.pod $(INST_MAN3DIR)/File::Slurp::Affix.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Lady Aleena <aleena@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: File-Slurp-Affix' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: 0.01_01' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "Lady Aleena <aleena@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "File-Slurp-Affix",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "0.01_01",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="File-Slurp-Affix" VERSION="0.01_01">' > File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Lady Aleena &lt;aleena@cpan.org&gt;</AUTHOR>' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Exporter::" />' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.28" />' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> File-Slurp-Affix.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> File-Slurp-Affix.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/File/Slurp/Affix.pm' 'blib/lib/File/Slurp/Affix.pm' \
	  'lib/File/Slurp/Affix.pod' 'blib/lib/File/Slurp/Affix.pod' 
